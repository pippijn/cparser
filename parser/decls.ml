...
let rec add_pointer_type decl =
  ...
  | IdentifierDeclarator _ ->
      TypedDecl (Traits.copy_pos decl, Sclass.empty, PointerType (EmptyType), decl, EmptyDecl, None)
  | TypedDecl (trs, sclasses, ty, decl, asm, init) ->
...
let rec set_base_type decl ty =
  ...
  | IdentifierDeclarator _ as decl ->
      TypedDecl (Traits.copy_pos decl, Sclass.empty, ty, decl, EmptyDecl, None)
...
let rec set_tspec decl spec =
  match decl with
  | EmptyDecl ->
      TypedDecl ([], Sclass.empty, spec, decl, EmptyDecl, None)
  | WildcardDecl _
  | IdentifierDeclarator _ ->
      TypedDecl (Traits.copy_pos decl, Sclass.empty, spec, decl, EmptyDecl, None)
  | StructDeclarator (trs, sdecl, bitfield) ->
      StructDeclarator (trs, set_tspec sdecl spec, bitfield)
  | TypedDecl (trs, sc, declty, untyped, EmptyDecl, None) when Sclass.is_empty sc ->
      TypedDecl (trs, Sclass.empty, Types.set_base_type spec declty, untyped, EmptyDecl, None)
...
let rec merge_decls spec decl =
  ...
  | TypedDecl (trs, sclasses, specty, IdentifierDeclarator (idtrs, ""), asm, init) ->
      Traits.add_attrs [Attributes.attribute idtrs] (merge_decls (TypedDecl (trs, sclasses, specty, EmptyDecl, asm, init)) decl)
  | TypedDecl (trs, sclasses, specty, EmptyDecl, asm, init) ->
      begin match decl with
      | TypedDecl (_, sc, declty, untyped, EmptyDecl, None) when Sclass.is_empty sc ->
...
let rec decl_base_type decl =
  ...
  | TypedDecl (trs, sclasses, declty, decl, asm, init) ->
      TypedDecl (trs, sclasses, Types.base_type declty, EmptyDecl, EmptyDecl, None)
...
let finish_decl decl asm init =
  match decl with
  | TypedDecl (trs, sclasses, declty, decl, EmptyDecl, None) ->
      let register =
